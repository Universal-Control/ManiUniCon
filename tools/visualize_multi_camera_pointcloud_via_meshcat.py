...
class MultiCameraPointCloudVisualizer:
    ...
    def __init__(
        ...
        for i, calib_file in enumerate(calib_files):
            ...
            if calib_data["extrinsics"] is None:
                print(
                    f"Warning: Camera {calib_data['camera_serial']} has no extrinsic data, skipping"
                )
                continue
    ...
    def setup_coordinate_frames(self):
        ...
        self.vis["frames"]["marker"].set_object(g.triad(scale=0.1))
        ...
        for i, calib in enumerate(self.calibrations):
            ...
            self.vis["frames"][camera_name].set_object(g.triad(scale=0.05))
        ...
        for i, camera in enumerate(self.cameras):
            try:
                ...
                if depth_frame and color_frame:
                    ...
                    if len(points) > 0:
                        points_marker = self.transform_pointcloud(
                            points, camera["extrinsics"]
                        )

                        # Choose whether to use original colors or camera-specific colors
                        use_original_colors = (
                            True  # Can be set to False to use camera-specific colors
                        )
                        ...
                        if use_original_colors:
                            ...
                            point_colors = np.tile(
                                camera_color.reshape(-1, 1), (1, len(points))
                            )

                        # Update point cloud in meshcat
                        self.vis[f"pointcloud{camera['index']+1}"].set_object(
                            g.PointCloud(
                                position=points_marker.T, color=point_colors, size=0.002
                            )
                        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-camera point cloud real-time visualization"
    )
    parser.add_argument(
        "calib_files",
        nargs="+",
