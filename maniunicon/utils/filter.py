...
class JointSpaceSmoother:
    ...
    def __init__(self, 
        ...
        # Initialize limits
        self.velocity_limit = velocity_limit if velocity_limit is not None else np.ones(num_joints) * 2.0
        self.acceleration_limit = acceleration_limit if acceleration_limit is not None else np.ones(num_joints) * 5.0
    ...
    def smooth(self, joint_positions: np.ndarray, timestamp: Optional[float] = None) -> np.ndarray:
        """
        ...
        dt = timestamp - self.last_timestamp if self.last_timestamp else 0.016  # Default 60Hz
    ...
    def _apply_velocity_limit(self, positions: np.ndarray, dt: float) -> np.ndarray:
        ...
        if self.last_position is not None and dt > 0:
            ...
            scale_factors = np.minimum(1.0, self.velocity_limit / (velocity_magnitude + 1e-6))
    ...
    def _apply_acceleration_limit(self, positions: np.ndarray, dt: float) -> np.ndarray:
        ...
        if self.last_position is not None and dt > 0:
            ...
            scale_factors = np.minimum(1.0, self.acceleration_limit / (acc_magnitude + 1e-6))
    ...
    def _apply_kalman_filter(self, positions: np.ndarray, dt: float) -> np.ndarray:
        ...
        filtered_positions = np.array([self.kf_state[i*2] for i in range(self.num_joints)])
    ...
    def _update_motion_statistics(self, positions: np.ndarray):
        ...
        if self.last_position is not None:
            ...
            self.motion_variance = (self.variance_alpha * instant_variance + 
...
class AdaptiveButterworth:
    ...
    def __init__(self, num_joints: int, cutoff_freq: float = 5.0, sample_rate: float = 60.0):
        """
...
if __name__ == "__main__":
    ...
    print(f"Input noise RMS: {np.sqrt(np.mean((noisy_positions - true_positions)**2)):.4f}")
    print(f"Smoothed error RMS: {np.sqrt(np.mean((smoothed_positions - true_positions)**2)):.4f}")
    print(f"Smoothing improvement: {(1 - np.sqrt(np.mean((smoothed_positions - true_positions)**2)) / np.sqrt(np.mean((noisy_positions - true_positions)**2))) * 100:.1f}%")