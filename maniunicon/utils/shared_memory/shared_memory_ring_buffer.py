...
class SharedMemoryRingBuffer:
    ...
    def get_last_k(self, k: int, out=None) -> Dict[str, np.ndarray]:
        ...
        if count == 0:
            # Return arrays filled with zeros if buffer is empty
            end_time = time.monotonic()
            dt = end_time - start_time
            if dt > self.get_time_budget:
                raise TimeoutError(f"Get time out {dt} vs {self.get_time_budget}")
        ...
        # Calculate how many actual items we have vs how many requested
        actual_items = min(count, k)
        padding_needed = k - actual_items
        ...
        for key, value in self.shared_arrays.items():
            ...
            if padding_needed > 0:
                # Find the oldest item index
                oldest_idx = (
                    curr_idx - count + 1 + self.buffer_size
                ) % self.buffer_size
                # Fill the beginning with repetitions of the oldest item
                for i in range(padding_needed):
                    target[i] = arr[oldest_idx]

            # Now fill the rest with actual data
            if actual_items <= curr_idx + 1:
                # All items fit without wraparound
                start_idx = curr_idx + 1 - actual_items
                target[padding_needed:k] = arr[start_idx : curr_idx + 1]
            else:
                # Need to handle wraparound
                # First part: from the wrapped portion at the end of buffer
                wrapped_items = actual_items - (curr_idx + 1)
                start_idx = self.buffer_size - wrapped_items
                target[padding_needed : padding_needed + wrapped_items] = arr[
                    start_idx : self.buffer_size
                ]

                # Second part: from the beginning of buffer to curr_idx
                if curr_idx >= 0:
                    target[padding_needed + wrapped_items : k] = arr[0 : curr_idx + 1]
