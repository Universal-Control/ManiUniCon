...
class FrankaPandaDeoxysInterface(RobotInterface):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        assert os.path.exists(
            self.deoxys_config_path
        ), "deoxys_config_path does not exist"
        ...
        self.controller_config_file = config.get(
            "controller_config", "osc-pose-controller.yml"
        )

        # Motion parameters
        self.action_scale = config.get(
            "action_scale", {"translation": 0.05, "rotation": 0.05}
        )
        ...
        try:
            ...
            if os.path.exists(self.controller_config_file):
                ...
                controller_cfg_path = os.path.join(
                    config_root, self.controller_config_file
                )
            ...
            if os.path.exists(controller_cfg_path):
                ...
                self.controller_cfg = get_default_controller_config(
                    self.default_controller_type
                )
            ...
            if hasattr(self.controller_cfg, "action_scale"):
                if "translation" in self.action_scale:
                    self.controller_cfg.action_scale.translation = self.action_scale[
                        "translation"
                    ]
                if "rotation" in self.action_scale:
                    self.controller_cfg.action_scale.rotation = self.action_scale[
                        "rotation"
                    ]
        ...
        try:
            ...
            if action.control_mode == "joint":
                if action.joint_positions is not None:
                    ...
                    target_q = np.clip(
                        target_q, self.joint_limits["min"], self.joint_limits["max"]
                    )
                    ...
                    gripper_action = 1.0 if self._gripper_state.item() else -1.0
                    deoxys_action = (action.joint_torques * 0.01).tolist() + [
                        gripper_action
                    ]
                ...
                if (
                    ...
                    deoxys_action = np.concatenate(
                        [pos_delta, axis_angle, [gripper_action]]
                    )
            ...
            if action.gripper_state is not None:
                if action.gripper_state.item() != self._gripper_state.item():
                    ...
                    print(
                        f"Gripper state updated to: {'closed' if action.gripper_state.item() else 'open'}"
                    )
