...
class UR5RobotiqInterface(RobotInterface):
    """UR5 robot plus Robotiq gripper interface with direct connection management."""

    def __init__(self, config: Dict[str, Any]):
        """Initialize the UR5 robot interface.
    ...
    def get_state(self) -> RobotState:
        ...
        try:
            # Get joint state
            joint_positions = self.robot_r.getActualQ()
            joint_velocities = self.robot_r.getActualQd()
            joint_torques = self.robot_r.getActualCurrent()
        ...
        try:
            ...
            if action.control_mode == "joint":
                ...
                if action.joint_positions is not None:
                    ...
                    self.robot_c.speedJ(
                        action.joint_velocities, self.acceleration, self.dt
                    )
                elif action.joint_torques is not None:
                    self.robot_c.speedJ(
                        action.joint_torques,  # UR5 uses speedJ for torque control
                        self.acceleration,
                        self.dt,
                    )
                ...
                if (
                    ...
                    current_q = np.array(self.robot_r.getActualQ())
    ...
    def stop(self) -> bool:
        ...
        try:
            self.robot_c.stopJ(self.acceleration)
            self.robot_c.servoStop()
            self.robot_c.stopScript()
            return True
        except Exception as e:
